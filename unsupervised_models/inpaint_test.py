...
def main():
    ...
    #im = imageio.imread('input/in000001.jpg',format='jpg')
    ...
    #mask
    files = glob.glob('results/badweather/skating/*.png')
    imagenames_list = []
    for f in files:
        imagenames_list.append(f)
    image_stack = []
    for n in range(0,40):
        image_in = cv.imread(imagenames_list[n],cv.IMREAD_GRAYSCALE)
        image_stack.append(image_in)

    #input
    files = glob.glob('skating/input/*.jpg')
    imagenames_list = []
    for f in files:
        imagenames_list.append(f)
    input_image_stack = []
    for n in range(0,40):
        ...
        input_image_stack.append(image_in)


    for n in range(0,40):
        # Inpaint.
        dst = cv.inpaint(input_image_stack[n], image_stack[n], 3, cv.INPAINT_NS)
        outfilename = 'inpainting_results/CV_NS/skating/res' + str(n) + '.png'
        # Write the output.
